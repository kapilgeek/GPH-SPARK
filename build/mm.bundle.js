...
//by Ocean adding academicsummary addon
...
.config(["$stateProvider", "$mmSideMenuDelegateProvider", "mmaAcademicsummaryPriority", function($stateProvider, $mmSideMenuDelegateProvider, mmaAcademicsummaryPriority) {
    $stateProvider
...
.run(["$log", "$mmaAcademicsummary", "$mmUtil", "$state", "$mmAddonManager", function($log, $mmaAcademicsummary, $mmUtil, $state, $mmAddonManager) {
...
//by geek adding missingassignment addon
angular.module('mm.addons.missingassignment', ['mm.core'])
.constant('mmaMissingassignmentListLimit', 50)
.constant('mmaMissingassignmentPriority', 800)
.config(["$stateProvider", "$mmSideMenuDelegateProvider", "mmaMissingassignmentPriority", function($stateProvider, $mmSideMenuDelegateProvider, mmaMissingassignmentPriority) {
    $stateProvider
...
.run(["$log", "$mmaMissingassignment", "$mmUtil", "$state", "$mmAddonManager", function($log, $mmaMissingassignment, $mmUtil, $state, $mmAddonManager) {
            ...
            if ($mmUtil.isTrueOrOne(missingassignment.notif)) {
                $mmaMissingassignment.isPluginEnabledForSite(missingassignment.site).then(function() {
                    $mmaMissingassignment.invalidateMissingassignmentList().finally(function() {
                        $state.go('redirect', {siteid: missingassignment.site, state: 'site.academicsummary'});
                        $state.go('redirect', {siteid: missingassignment.site, state: 'site.academicsummary'});
                    });
                });
                return true;
...
angular.module('mm.addons.grades')
.factory('$mmaGradesHandlers', ["$mmaGrades", "$state", function($mmaGrades, $state) {
    var self = {};
...
//by Ocean for adding academicsummary addon
angular.module('mm.addons.academicsummary')
.controller('mmaAcademicsummaryListCtrl', ["$scope", "$mmUtil", "$mmaAcademicsummary", "mmaAcademicsummaryListLimit", "$sce", "$compile", "$state", function($scope, $mmUtil, $mmaAcademicsummary, mmaAcademicsummaryListLimit, $sce, $compile, $state) {
       ...
       /* return $mmaAcademicsummary.getUnreadAcademicsummary(unreadCount, mmaAcademicsummaryListLimit).then(function(unread) {
            ...
            if (unread.length < mmaAcademicsummaryListLimit) {
                ...
                return $mmaAcademicsummary.getReadAcademicsummary(readCount, readLimit).then(function(read) {
                    ...
                    $scope.canLoadMore = read.length >= readLimit;
                    var uncompiled = $sce.trustAsHtml($scope.academicsummary[0].summary);

                //var compliled = $compile(uncompiled);
               // $scope.summaryHTML = $sce.trustAsHtml($scope.academicsummary[0].summary);
             // alert(compliled);

                    $scope.uncompiled = uncompiled;
                          //var template = "<button ng-click='doSomething()'>{{label}}</button>";
                            var content = $compile($scope.uncompiled)($scope);
                            //var content = linkFn($scope);
                            alert(content);
                            $scope.summaryHTML = content;
                            //element.append(content);
                      

             // $scope.summaryHTML = uncompiled;
             // $compile( $scope.summaryHTML )($scope);
             ...
             // }
                ...
                //console.log('asdf');
                //console.log($scope.academicsummary[0].summary);
                var uncompiled = $sce.trustAsHtml($scope.academicsummary[0].summary);
                //var compliled = $compile(uncompiled);
               // $scope.summaryHTML = $sce.trustAsHtml($scope.academicsummary[0].summary);
             // alert(compliled);
...
angular.module('mm.addons.academicsummary')
.directive('mmaAcademicsummaryActions', ["$log", "$mmModuleActionsDelegate", "$state", function($log, $mmModuleActionsDelegate, $state) {
...
angular.module('mm.addons.academicsummary')
.filter('mmaAcademicsummaryFormat', ["$mmText", function($mmText) {
  return function(text) {
    text = text.replace(/-{4,}/ig, '');
    text = $mmText.replaceNewLines(text, '<br />');
    return text;
...
angular.module('mm.addons.academicsummary')
.factory('$mmaAcademicsummaryHandlers', ["$log", "$mmaAcademicsummary", function($log, $mmaAcademicsummary) {
...
angular.module('mm.addons.academicsummary')
.factory('$mmaAcademicsummary', ["$q", "$log", "$mmSite", "$mmSitesManager", "mmaAcademicsummaryListLimit", function($q, $log, $mmSite, $mmSitesManager, mmaAcademicsummaryListLimit) {
    $log = $log.getInstance('$mmaAcademicsummary');
    var self = {};
    ...
    self.getAcademicsummary = function(read, limitFrom, limitNumber) {
        ...
        return $mmSite.read('core_message_get_messages', data, preSets).then(function(response) {
            ...
            if (response.summary) {
                ...
                console.log("academicsummary"+ academicsummary);
...
angular.module('mm.addons.missingassignment')
.directive('mmaMissingassignmentActions', ["$log", "$mmModuleActionsDelegate", "$state", function($log, $mmModuleActionsDelegate, $state) {
    $log = $log.getInstance('mmaMissingassignmentActions');
    function link(scope, element, attrs) {
        if (scope.contexturl) {
            scope.actions = $mmModuleActionsDelegate.getActionsFor(scope.contexturl, scope.courseid);
        }
...
controller.$inject = ["$scope"];
    return {
        ...
        templateUrl: 'addons/missingassignment/templates/actions.html',
    };
}]);
angular.module('mm.addons.missingassignment')
.filter('mmaMissingassignmentFormat', ["$mmText", function($mmText) {
  return function(text) {
    text = text.replace(/-{4,}/ig, '');
    text = $mmText.replaceNewLines(text, '<br />');
    return text;
...
angular.module('mm.addons.missingassignment')
.factory('$mmaMissingassignmentHandlers', ["$log", "$mmaMissingassignment", function($log, $mmaMissingassignment) {
...
angular.module('mm.addons.missingassignment')
.factory('$mmaMissingassignment', ["$q", "$log", "$mmSite", "$mmSitesManager", "mmaMissingassignmentListLimit", function($q, $log, $mmSite, $mmSitesManager, mmaMissingassignmentListLimit) {
    $log = $log.getInstance('$mmamissingassignment');
    var self = {};
    ...
    self.getMissingassignment = function(read, limitFrom, limitNumber) {        
        ...
        return $mmSite.read('core_message_get_messages', data, preSets).then(function(response) {        
            ...
            if (response.massign) {
                ...
                //console.log("missingassignment"+ missingassignment);
